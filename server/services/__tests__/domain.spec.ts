...
import { batches, cellarMembers, cellars, measurements, transferDestinations, transfers, users, vessels, wines } from '../../database/schema'
import { createBatch, closeBatch, forceDeleteBatch, getBatch } from '../batch.service'
...
beforeEach(() => {
  ...
  context.db.insert(vessels).values([
    { id: 'v1', cellarId: 'cellar-1', name: 'Zdroj', type: VesselType.STEEL_TANK, capacity: 200 },
    { id: 'v2', cellarId: 'cellar-1', name: 'Cieľ A', type: VesselType.STEEL_TANK, capacity: 100 },
    { id: 'v3', cellarId: 'cellar-1', name: 'Cieľ B', type: VesselType.OAK_BARREL, capacity: 100 },
    { id: 'v4', cellarId: 'cellar-1', name: 'Cieľ C', type: VesselType.DEMIJOHN, capacity: 20 },
  ]).run()
})

afterEach(() => context.sqlite.close())

function insertBatch(phase: BatchPhase, volume = 100, vesselId = 'v1', id = `2026-IO-${phase}-001`) {
  context.db.insert(batches).values({ id, cellarId: 'cellar-1', wineId: 'wine-1', phase, vesselId, volume, status: BatchStatus.ACTIVE, openedAt: new Date('2026-08-01T00:00:00Z') }).run()
  return id
}
...
describe('batch lifecycle services', () => {
  it('generuje deterministické ID v scope pivnica/ročník/víno/fáza', async () => {
    const first = await createBatch(context.db, 'cellar-1', { wineId: 'wine-1', vesselId: 'v1', volume: 100 })
    const second = await createBatch(context.db, 'cellar-1', { wineId: 'wine-1', vesselId: 'v2', volume: 80 })
  ...
  it('vykoná odkalenie v jednej transakcii a zachová parent lineage', () => {
    ...
    const result = transferBatch(context.db, transferContext, { sourceBatchId: sourceId, targetPhase: BatchPhase.CLARIFICATION, destinations: [{ vesselId: 'v2', volume: 95 }], lossVolume: 5 })
    ...
    expect(child).toMatchObject({ phase: BatchPhase.CLARIFICATION, parentBatchId: sourceId, volume: 95 })
  })

  it('vykoná single-destination stáčanie do kvasenia', () => {
    const sourceId = insertBatch(BatchPhase.CLARIFICATION)
    const result = transferBatch(context.db, transferContext, { sourceBatchId: sourceId, targetPhase: BatchPhase.FERMENTATION, destinations: [{ vesselId: 'v2', volume: 98 }], lossVolume: 2 })
  ...
  it('rozdelí šaržu do viacerých nádob a priradí každú k transferu', () => {
    const sourceId = insertBatch(BatchPhase.CLARIFICATION)
    const result = transferBatch(context.db, transferContext, { sourceBatchId: sourceId, targetPhase: BatchPhase.FERMENTATION, destinations: [{ vesselId: 'v2', volume: 60 }, { vesselId: 'v3', volume: 35 }], lossVolume: 5 })
  ...
  it('odmietne neúplnú objemovú bilanciu bez čiastočných zmien', () => {
    const sourceId = insertBatch(BatchPhase.CLARIFICATION)
    expect(() => transferBatch(context.db, transferContext, { sourceBatchId: sourceId, targetPhase: BatchPhase.FERMENTATION, destinations: [{ vesselId: 'v2', volume: 80 }], lossVolume: 5 })).toThrow('zodpovedať')
  ...
  it('odmietne prekročenie kapacity cieľovej nádoby', () => {
    ...
    expect(() => transferBatch(context.db, transferContext, { sourceBatchId: sourceId, targetPhase: BatchPhase.FERMENTATION, destinations: [{ vesselId: 'v2', volume: 120 }], lossVolume: 0 })).toThrow('kapacitu')
  })

  it('uchová rekonštruovateľnú lineage cez batch aj transfer destination', () => {
    const sourceId = insertBatch(BatchPhase.FERMENTATION)
    const result = transferBatch(context.db, transferContext, { sourceBatchId: sourceId, targetPhase: BatchPhase.AGING, destinations: [{ vesselId: 'v2', volume: 100 }], lossVolume: 0 })
  ...
  it('force delete vyžaduje frázu a chráni naviazané dáta', async () => {
    ...
    const emptyId = insertBatch(BatchPhase.MUST, 10, 'v4', '2026-IO-MUST-099')
